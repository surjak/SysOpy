...
void pack_order()
{
    ...
    semop(semafore_id, load, 4);

    orders *ord = shmat(shared_memory_id, NULL, 0);

    int index = (semctl(semafore_id, 2, GETVAL, NULL) - 1) % MAX_ORDERS;
    ord->values[index] *= 2;
    int orders_to_prepare = semctl(semafore_id, 3, GETVAL, NULL);
    int orders_to_send = semctl(semafore_id, 5, GETVAL, NULL) + 1;
...
int main()
{
    ...
    {
        usleep(rand_time);
        if (semctl(semafore_id, 3, GETVAL, NULL) > 0)
        {
            pack_order();
        }
    }
